...
std::vector<int> Network::suggestFriends(int who, int &score)
{
    ...
    for (int id : whoSet) { //Who user's friends ids
        ...
        for (int friendID : whoFriend->getFriends()) { //Friends of Who's friends
            if (whoSet.find(friendID) == whoSet.end() && scores[friendID] == 0
                && friendID
                       != who) { //Checks to make sure they aren't already friends and they weren't given a score yet
    ...
    for (int i = 0; i < scores.size(); i++) {
        if (scores[i] == score
            && whoSet.find(i) == whoSet.end()) { //If whoset is a set of aled montes's friends, then
...
int Network::findScore(std::set<int> friendsList, int id)
{
    ...
    for (int friendID : user->getFriends()) {
        if (friendsList.find(friendID)
            != friendsList
                   .end()) { //friendsList is aled montes's friends, id is a friend of aled's friend's id
            score++;
        }
    }
    return score;
...
void Network::addPost(int ownerId,
                      std::string message,
                      int likes,
                      bool isIncoming,
